...
url= "https://www.ulta.com/natural-skin-care?N=ufu4&No=0&Nrpp=96"
...
file_csv = csv.writer(open('Ulta_Natural-Skincare_{}.csv'.format(date.today()), 'w'))
file_csv.writerow(['Brand', 'Product', 'Ingredients', 'Image Link','Item Link'])

counter = 96 # needs to be multiples of 48 until 912 (non-inclusive for 864)
...
while counter < 288:
    ...
    def next_page():
        global counter
        url = "https://www.ulta.com/natural-skin-care?N=ufu4&No={}&Nrpp=96".format(counter)
        driver.get(url)
        counter += 96
        time.sleep(3)
    get_info()
